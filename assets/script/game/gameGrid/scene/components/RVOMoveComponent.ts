...
export class RVOMoveComponent extends EntityComponent {
    ...
    protected updateVo(): void {
        this.entity.setPosition(this._vo.pos);
    ...
    protected moving(): void {
        ...
        this.RVOMoving();
        this._vo.updatePos(this.entity.position,this.entity.worldPosition);
    ...
    private RVOMoving() {
        ...
        if(sid >= 0){
            ...
            this.entity.position = new Vec3(pos.x, 0, pos.y);
            if (Math.abs(vel.x) > 0.01 && Math.abs(vel.y) > 0.01) {
                this.entity.forward = new Vec3(vel.x, 0, vel.y).normalize();
            }
        }
