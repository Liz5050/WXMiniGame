...
import { GameGridRoundType } from "db://assets/script/cache/GameGridCache";
import { CacheManager } from "db://assets/script/manager/CacheManager";
import { Root3D } from "db://assets/script/Root3D";
import { GameGridMapView } from "../../view/GameGridMapView";
...
export default class GameEntityLayer extends BaseLayer{
    ...
    private _gridEntitys:BaseEntity[][];
    onInit(){
    ...
    private initMapGrid(){
        ...
        for(let row = 0; row < 10; row++){
            for(let col = 0; col < 10; col++){
                if(!this._gridEntitys[row]){
                    this._gridEntitys[row] = [];
                }
                let mapItemNode = GameFactory.getEntity(EntityType.Grid);
                mapItemNode.name = `Grid_${col}_${row}`;
                this.addChild(mapItemNode);
                this._gridEntitys[row][col] = mapItemNode;
            }
        }
    ...
    @msg(GEvent.OnGridItemTouchMove)
    private onGridMove(data:{touchX:number,touchY:number}){
